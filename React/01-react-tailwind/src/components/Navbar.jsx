...
export default function Navbar() {
    ...
    return (
      <div className="text-white flex items-center h-24 max-w-[1240px] px-4 mx-auto">
        ...
        <div onClick = {handleNav} className='block z-10 hover:cursor-pointer md:hidden'>
            {/* if funkce pro rozhodnuti jaka ikona se zobrazi */}
            {nav ? <div className='fixed top-9 right-4'><AiOutlineClose size={20} /></div> : <div className=""><AiOutlineMenu size={20} /></div>}
        </div>
        {/* if funkce pro vykresleni vyskakujici navigace u mobilu pri aktivnim hamburger menu */}
        <div className={nav ? 'fixed left-0 top-0 border-r border-r-gray-900 h-full sm:w-[40%] bg-[#000300] ease-in-out duration-500 w-full md:hidden' : 'fixed left-[-100%] md:ease-in-out md:duration-500'}>
